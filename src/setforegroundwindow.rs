...
// set_foreground_window_terminal works though.
...
// too much work.
extern "system" fn set_foreground_window_remote(_windowh: HWND) -> BOOL {
    unsafe {
        let _instance = LoadLibraryA(PCSTR(b"user32.dll\0".as_ptr()));
...
unsafe fn attempt_set_foreground(target_window: HWND, fore_window: HWND) -> HWND {
	...
	let _result = SetForegroundWindow(target_window);
